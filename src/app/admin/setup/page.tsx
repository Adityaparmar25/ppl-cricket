...
export default function SetupPage() {
  ...
  // Step 2 — toss determines batting first
  ...
  useEffect(() => {
    fetch('/api/match')
      ...
      .then((d: MatchState) => {
        setMatch(d);
        if (d.status !== 'SETUP' && d.status !== 'TOSS') {
          router.push('/admin');
        }
      });
  }, [router]);
  ...
  // Batting first is determined by toss — who won AND what they chose
  const battingTeamId =
    tossDecision === 'bat'
      ? tossWonBy
      : tossWonBy === team1?.id
  ...
  async function handleConfig() {
    const ok = await post('/api/admin/setup', { totalOvers: overs, venue,
      battingFirstId: team1?.id, // placeholder, will be overwritten after toss
    });
    if (ok) setStep('toss');
  }
  ...
  async function handleStart() {
    ...
    if (opener1 === opener2)   { setError('Both openers cannot be the same player'); return; }
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        ...
        <div className="flex gap-2">
          {STEPS.map((s, i) => (
            <div key={s} className={`flex-1 h-1.5 rounded-full transition-all ${
              i < stepIdx ? 'bg-[#F5A623]' : i === stepIdx ? 'bg-[#E8510A]' : 'bg-white/10'
            }`} />
          ))}
        </div>
        ...
        {step === 'config' && (
          <div className="bg-[#1A1A1A] border border-white/10 rounded-xl p-5 space-y-5">
                ...
                {[5, 10, 15, 20].map((o) => (
                  <button key={o} onClick={() => setOvers(o)}
                    className={`px-5 py-2 rounded-lg font-bold text-sm transition-colors ${
                      overs === o ? 'bg-[#E8510A] text-white' : 'bg-white/5 text-gray-300 hover:bg-white/10'
                    }`}>
                    {o}
                  </button>
                ))}
            ...
            {/* Venue */}
            <div>
              <p className="text-xs text-gray-400 uppercase tracking-wider mb-2">Venue (optional)</p>
              <input type="text" placeholder="e.g. PPL Ground, Jharkhand" value={venue}
          ...
          <div className="bg-[#1A1A1A] border border-white/10 rounded-xl p-5 space-y-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[team1, team2].map((t) => t && (
                  <button key={t.id} onClick={() => setTossWonBy(t.id)}
                    className={`py-3 px-4 rounded-xl font-semibold text-sm transition-colors text-left ${
                      tossWonBy === t.id ? 'bg-[#F5A623] text-black' : 'bg-white/5 text-gray-300 hover:bg-white/10'
                    }`}>
                    🪙 {t.name}
                  </button>
                ))}
                ...
                {(['bat', 'bowl'] as const).map((d) => (
                  <button key={d} onClick={() => setTossDecision(d)}
                    className={`py-3 rounded-xl font-bold capitalize transition-colors ${
                      tossDecision === d ? 'bg-[#F5A623] text-black' : 'bg-white/5 text-gray-300 hover:bg-white/10'
                    }`}>
                    {d === 'bat' ? '🏏 Bat' : '⚡ Bowl'}
                  </button>
                ))}
        ...
        {step === 'openers' && battingTeam && bowlingTeam && (
          <div className="bg-[#1A1A1A] border border-white/10 rounded-xl p-5 space-y-5">
            ...
            <div className="flex gap-3">
              <button onClick={() => { setStep('toss'); setError(''); }}
                className="flex-1 py-3 rounded-xl bg-white/5 text-gray-300 font-semibold hover:bg-white/10">
                ← Back
              </button>
...
// ── Player dropdown with dark styling ─────────────────────────────────────────

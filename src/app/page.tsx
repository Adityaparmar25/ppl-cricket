...
export default function PublicScorePage() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <p className="text-[#E8510A] text-xl font-bold animate-pulse">Loading...</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-lg mx-auto px-3 py-6 space-y-4">
        <div className="card-dark p-6 text-center">
          ...
          <h1 className="text-2xl font-black text-[#F5A623]">Match Complete!</h1>
          {match.resultText && (
            <p className="text-white font-semibold mt-2">{match.resultText}</p>
          )}
        </div>
        {[match.innings1, match.innings2].map((inn, i) => inn && (
          <div key={i} className="card-dark p-4 flex justify-between items-center">
            <span className="text-gray-300 text-sm">
              {inn.battingTeamId === match.team1.id ? match.team1.name : match.team2.name}
            </span>
            <span className="text-white font-black text-xl">
              {inn.totalRuns}/{inn.wickets}
              <span className="text-gray-500 text-xs font-normal ml-1">({inn.overs} Ov)</span>
            </span>
          </div>
        ))}
      </div>
  ...
  return (
    <main className="max-w-lg mx-auto px-3 pb-10">
      ...
      {!connected && (
        <div className="bg-red-900/40 border border-red-500/30 text-red-400
      ...
      <div className="sticky top-0 z-10 bg-[#0D0D0D] pt-3 pb-2 space-y-2">
        ...
        <MatchStatusBanner status={match.status} resultText={match.resultText ?? undefined} />
      </div>

      {currentInnings && (
        <div className="card-dark p-4 mt-3">
          <p className="text-[10px] text-gray-500 uppercase tracking-widest mb-2">This Over</p>
      ...
      {currentInnings && currentInnings.currentBatsmen.length > 0 && (
        <div className="card-dark mt-3">
          <p className="text-[10px] text-gray-500 uppercase tracking-widest px-4 pt-3 pb-1">Batting</p>
      ...
      {currentInnings?.currentBowler && (
        <div className="card-dark mt-3">
          <p className="text-[10px] text-gray-500 uppercase tracking-widest px-4 pt-3 pb-1">Bowling</p>
      ...
      {currentInnings && currentInnings.fallOfWickets.length > 0 && (
        <div className="card-dark p-4 mt-3">
          <p className="text-[10px] text-gray-500 uppercase tracking-widest mb-2">Fall of Wickets</p>
      ...
      {match.currentInnings === 2 && match.innings1 && (
        <div className="card-dark p-4 mt-3 flex justify-between items-center text-sm">
          <span className="text-gray-400">
            {match.innings1.battingTeamId === match.team1.id ? match.team1.name : match.team2.name}
          </span>
          <span className="text-[#F5A623] font-black">
            {match.innings1.totalRuns}/{match.innings1.wickets}
            <span className="text-gray-500 font-normal ml-1 text-xs">({match.innings1.overs} Ov)</span>
          </span>
        </div>
      )}
      ...
      {lastUpdate && (
        <p className="text-center text-gray-700 text-xs mt-4">
          Updated {lastUpdate.toLocaleTimeString()} · refreshes every {match.status === 'LIVE' ? '3' : '15'}s
        </p>
      )}
    </main>
  );
